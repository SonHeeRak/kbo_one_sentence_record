...
import pandas as pd
import re
from article.lib.korean import lab2ai_linguistic as linguistic
...
class SentenceGenerator(object):
    ...
    def set_variable(self, var, table_name):
        ...
        for d in df_dynamic_group:
            ...
            condition = self.get_global_condition(var_dict['condition'], var)
            if condition:
                str_sentence = self.get_result_string(self.main_dict, var_dict['sentence'], var)
                if var_dict['eval'] == 'T':
    ...
    def get_dynamic_variable(self, val, variable):
        ...
        for var_dict in variable_result:
            cond = self.get_condition(var_dict['condition'], val)
            if cond:
                str_sentence = self.get_result_string(param_dict, var_dict['sentence'], val)
